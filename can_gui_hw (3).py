...
# ── Vector XL channel configuration ───────────────────────────────────────────
# VN1640A physical channel labels vs python-can Vector XL channel index:
#   The Vector XL driver numbers channels 0-based from what Vector Hardware Config
#   assigns.  For a VN1640A with 4 channels:
#     Physical CH1 (LIN) → xl index 0
#     Physical CH2 (LIN) → xl index 1
#     Physical CH3 (CAN) → xl index 2
#     Physical CH4 (CAN) → xl index 3
#
#   python-can "vector" bustype accepts channel as an integer (xl channel index)
#   OR as a channel mask string.  We use integer indices here.
#
#   If detection fails, run probe_vector_channels() (see CANWorker) and read
#   the printed output to find the correct indices for your setup.
#
#   Change these two values if your hardware enumerates differently:
CH_DUT = 2   # Physical CH3 → DUT
CH_PS  = 3   # Physical CH4 → PS1 + PS2

# App name must match one registered in Vector Hardware Config, or use "Python"
# which is always registered automatically by the XL driver installer.
VECTOR_APP_NAME = "Python"
...
PS_WRITE_SIGNALS = [
    ...
    ("DC_Current_Fall (s)", "SDO",   "SDO_Server_DC_Current_Fall",   0,-999,999,0.001,"s"),
    ("DC_Current_Rise (s)", "SDO",   "SDO_Server_DC_Current_Rise",   0,-999,999,0.001,"s"),
    ("DC_Voltage_Fall (s)", "SDO",   "SDO_Server_DC_Voltage_Fall",   0,-999,999,0.001,"s"),
    ("DC_Voltage_Rise (s)", "SDO",   "SDO_Server_DC_Voltage_Rise",   0,-999,999,0.001,"s"),
    ("DC_Output_Res (Ω)",   "SDO",   "SDO_Server_DC_Output_Resistance",0,-999,999,0.001,"Ω"),
...
DUT_READ_SIGNALS = [
    ...
    ("DCDC12vActualCurrent",  "HVDCDC_PMZ_12v", "DCDC12vActualCurrent"),
    ("DCDC12vActualVoltage",  "HVDCDC_PMZ_12v", "DCDC12vActualVoltage"),
]
...
DUT_WRITE_SIGNALS = [
    ...
    ("DCDC12vSetpointRequest",   "GWM_PMZ_CoreSystem",  "DCDC12vSetpointRequest",0,  0,     25.3, 0.1, "V"),
...
# ═══════════════════════════════════════════════════════════════════════════════
#  Channel probe helper  (call this standalone to find your XL channel indices)
# ═══════════════════════════════════════════════════════════════════════════════
def probe_vector_channels():
    """
    Try opening each Vector XL channel index 0-7 and print which ones work.
    Run this function once (uncomment the call at the bottom of main()) to
    discover the correct CH_DUT / CH_PS values for your VN1640A setup.
    """
    if not HW_AVAILABLE:
        print("python-can not installed"); return
    print("\n── Vector XL channel probe ──────────────────────────────")
    for idx in range(8):
        try:
            b = can.interface.Bus(bustype="vector", channel=idx,
                                  bitrate=500000, app_name=VECTOR_APP_NAME)
            print(f"  index {idx}  → OK  (opened successfully)")
            b.shutdown()
        except Exception as e:
            # Filter noise: only print if it's not a simple "channel not present"
            msg = str(e)
            if "not found" in msg.lower() or "invalid" in msg.lower() or "no such" in msg.lower():
                print(f"  index {idx}  → not present")
            else:
                print(f"  index {idx}  → error: {msg}")
    print("─────────────────────────────────────────────────────────\n")


# ═══════════════════════════════════════════════════════════════════════════════
#  CAN Worker  –  CH_DUT = DUT (bus_dut),  CH_PS = PS1+PS2 (bus_ps)
# ═══════════════════════════════════════════════════════════════════════════════
class CANWorker(QObject):
    ...
    bus_status = pyqtSignal(int, bool)   # (channel_physical, ok)

    def __init__(self, dbc_ps1, dbc_ps2, dbc_dut):
        super().__init__()
        self.dbc_ps1 = dbc_ps1; self.dbc_ps2 = dbc_ps2; self.dbc_dut = dbc_dut
        self._running = False
        self.bus_dut  = None   # CH_DUT – DUT
        self.bus_ps   = None   # CH_PS  – PS1 + PS2
        self._ready   = threading.Event()   # set once buses are attempted

    def start_buses(self):
        """Open each bus independently so one failure does not block the other."""
        # ── DUT bus ─────────────────────────────────────────────────────────
        try:
            self.bus_dut = can.interface.Bus(
                bustype="vector",
                channel=CH_DUT,
                bitrate=500000,
            ...
            self.status.emit(f"CH{CH_DUT+1}/DUT bus connected (xl index {CH_DUT})")
            self.bus_status.emit(3, True)
        except Exception as e:
            self.bus_dut = None
            self.status.emit(f"CH{CH_DUT+1}/DUT bus FAILED (xl index {CH_DUT}): {e}")
            self.bus_status.emit(3, False)

        # ── PS bus ───────────────────────────────────────────────────────────
        try:
            self.bus_ps = can.interface.Bus(
                bustype="vector",
                channel=CH_PS,
                bitrate=500000,
                app_name=VECTOR_APP_NAME,
            ...
            self.status.emit(f"CH{CH_PS+1}/PS bus connected (xl index {CH_PS})")
            self.bus_status.emit(4, True)
        except Exception as e:
            self.bus_ps = None
            self.status.emit(f"CH{CH_PS+1}/PS bus FAILED (xl index {CH_PS}): {e}")
            self.bus_status.emit(4, False)

        self._running = (self.bus_dut is not None or self.bus_ps is not None)
        self._ready.set()   # unblock receive_loop
    ...
    def receive_loop(self):
        """Block until buses are initialised, then poll both."""
        self._ready.wait(timeout=10.0)   # wait up to 10 s for start_buses()
        while self._running:
            # DUT bus
            if self.bus_dut:
                try:
                    ...
                    self.status.emit(f"RX DUT error: {e}"); time.sleep(0.1)
            # PS bus
            if self.bus_ps:
                try:
                    ...
                    self.status.emit(f"RX PS error: {e}"); time.sleep(0.1)
...
class StatusBar(QWidget):
    def __init__(self):
        ...
        self._ch_dut = QLabel(f"CH{CH_DUT+1}/DUT: ●")
        self._ch_ps  = QLabel(f"CH{CH_PS+1}/PS: ●")
        self._tx     = QLabel("TX: –")
        self._ch_dut.setStyleSheet("color:#ff4444")
        self._ch_ps.setStyleSheet("color:#ff4444")
        lay.addWidget(self._ch_dut); lay.addWidget(self._ch_ps); lay.addWidget(self._tx, 1)

    def set_connected(self, physical_ch, ok):
        lbl = self._ch_dut if physical_ch == 3 else self._ch_ps
        name = "DUT" if physical_ch == 3 else "PS"
        lbl.setText(f"CH{physical_ch}/{name}: ●")
...
class MainWindow(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle(f"CAN GUI  –  VN1640A  |  CH{CH_DUT+1}→DUT  CH{CH_PS+1}→PS1+PS2")
        ...
        self._worker.bus_status.connect(self._on_bus_status)
        self._worker.status.connect(lambda m: self.statusBar().showMessage(m, 8000))
        self._worker_thread.started.connect(self._worker.start_buses)
        self._worker_thread.start()

        # Receive loop starts in a daemon thread.
        # CANWorker._ready event ensures it waits until start_buses() has run.
        ...
        # Build diag tab last so it can reference CH_DUT/CH_PS labels
        self._build_diag_tab()

    def _on_bus_status(self, physical_ch: int, ok: bool):
        self._status_bar.set_connected(physical_ch, ok)
    ...
    def _build_remote_tab(self):
        ...
        self._ps1 = PSSection(f"⚡  Power Supply 1  (IT6000 – CH{CH_PS+1}/CAN2)", 1)
        self._ps2 = PSSection(f"⚡  Power Supply 2  (IT6000 – CH{CH_PS+1}/CAN2)", 2)
    ...
    def _build_diag_tab(self):
        """Channel probe / connection diagnostics tab."""
        tab = QWidget(); self._tabs.addTab(tab, "🔌  HW Diagnostics")
        lay = QVBoxLayout(tab); lay.setSpacing(10)

        # Current config display
        cfg_grp = QGroupBox("Current Channel Configuration")
        cfg_lay = QGridLayout(cfg_grp)
        cfg_lay.addWidget(QLabel("DUT bus  (xl index / physical CH):"), 0, 0)
        self._cfg_dut_lbl = QLabel(f"index {CH_DUT}  →  Physical CH{CH_DUT+1}")
        self._cfg_dut_lbl.setObjectName("readVal"); cfg_lay.addWidget(self._cfg_dut_lbl, 0, 1)
        cfg_lay.addWidget(QLabel("PS bus   (xl index / physical CH):"), 1, 0)
        self._cfg_ps_lbl = QLabel(f"index {CH_PS}  →  Physical CH{CH_PS+1}")
        self._cfg_ps_lbl.setObjectName("readVal"); cfg_lay.addWidget(self._cfg_ps_lbl, 1, 1)
        cfg_lay.addWidget(QLabel("App name:"), 2, 0)
        app_lbl = QLabel(VECTOR_APP_NAME); app_lbl.setObjectName("readVal")
        cfg_lay.addWidget(app_lbl, 2, 1)
        lay.addWidget(cfg_grp)

        # Channel index override
        override_grp = QGroupBox("Override Channel Indices  (takes effect on Reconnect)")
        ov_lay = QGridLayout(override_grp)
        ov_lay.addWidget(QLabel("DUT xl index:"), 0, 0)
        self._dut_idx_spin = QSpinBox(); self._dut_idx_spin.setRange(0, 15); self._dut_idx_spin.setValue(CH_DUT)
        ov_lay.addWidget(self._dut_idx_spin, 0, 1)
        ov_lay.addWidget(QLabel("PS xl index:"), 1, 0)
        self._ps_idx_spin = QSpinBox(); self._ps_idx_spin.setRange(0, 15); self._ps_idx_spin.setValue(CH_PS)
        ov_lay.addWidget(self._ps_idx_spin, 1, 1)
        reconnect_btn = QPushButton("🔁  Apply & Reconnect")
        reconnect_btn.setObjectName("startBtn"); reconnect_btn.clicked.connect(self._reconnect_buses)
        ov_lay.addWidget(reconnect_btn, 2, 0, 1, 2)
        lay.addWidget(override_grp)

        # Channel probe
        probe_grp = QGroupBox("Channel Probe  –  find which xl indices are available")
        probe_lay = QVBoxLayout(probe_grp)
        probe_btn = QPushButton("🔍  Probe All Vector XL Channels (0–7)")
        probe_btn.setObjectName("diagBtn"); probe_btn.clicked.connect(self._probe_channels)
        probe_lay.addWidget(probe_btn)
        self._probe_log = QTextEdit(); self._probe_log.setReadOnly(True)
        self._probe_log.setPlaceholderText("Probe results will appear here…")
        probe_lay.addWidget(self._probe_log)
        lay.addWidget(probe_grp)
        lay.addStretch()

    def _probe_channels(self):
        """Run channel probe in a background thread and report results."""
        self._probe_log.clear()
        self._probe_log.append("Probing Vector XL channels 0–7 …\n")
        def _run():
            results = []
            for idx in range(8):
                try:
                    b = can.interface.Bus(bustype="vector", channel=idx,
                                         bitrate=500000, app_name=VECTOR_APP_NAME, fd=False)
                    results.append((idx, True, "OK – channel available"))
                    b.shutdown()
                except Exception as e:
                    results.append((idx, False, str(e)[:120]))
            return results
        def _done(results):
            for idx, ok, msg in results:
                colour = "#00ff88" if ok else "#4a6080"
                self._probe_log.append(
                    f'<span style="color:{colour}">  xl index {idx}  (Physical CH{idx+1}):  {msg}</span>')
            self._probe_log.append("\n<span style='color:#ffdd00'>▶ Set the DUT and PS indices above and click Reconnect.</span>")
        import concurrent.futures
        fut = concurrent.futures.ThreadPoolExecutor(max_workers=1).submit(_run)
        def _poll():
            if fut.done():
                _done(fut.result())
            else:
    ...
    def _reconnect_buses(self):
        """Apply new channel indices and reopen buses."""
        global CH_DUT, CH_PS
        self._tx_timer.stop()
        self._worker.stop_buses()
        time.sleep(0.3)
        CH_DUT = self._dut_idx_spin.value()
        CH_PS  = self._ps_idx_spin.value()
        self._cfg_dut_lbl.setText(f"index {CH_DUT}  →  Physical CH{CH_DUT+1}")
        self._cfg_ps_lbl.setText(f"index {CH_PS}  →  Physical CH{CH_PS+1}")
        self._worker._running = False
        self._worker._ready.clear()
        self._worker.bus_dut = None; self._worker.bus_ps = None
        self._worker.start_buses()
        self._tx_timer.start(15)
        self.statusBar().showMessage(f"Reconnected with DUT=xl{CH_DUT} PS=xl{CH_PS}", 5000)
...
def main():
    # Run with  --probe  to discover channel indices without opening the GUI:
    #   python can_gui_hw.py --probe
    if "--probe" in sys.argv:
        probe_vector_channels()
        return
